#include "cartographer_ros/error_state_kalman/initializer.h"
...
bool Initializer::AddLaserData(const LaserPoseDataPtr laser_data_ptr, State *state) {
    if (imu_buffer_.size() < kImuDataBufferLength) {
    ...
    if (std::abs(laser_data_ptr->timestamp - last_imu_ptr->timestamp) > 0.5) {
    ...
    // InitOrientationFromLases(&state->G_R_I, laser_data_ptr);
    state->G_R_I = laser_data_ptr->orientation.matrix();
    state->G_p_I = laser_data_ptr->position;
...
bool Initializer::InitOrientationFromLases(Eigen::Matrix3d* G_R_I, const LaserPoseDataPtr laser_pose) {
    Eigen::Matrix3d R (laser_pose->orientation);
    *G_R_I = R;
    return true;
}
